...
bmi.exclusion <- get_redcap() %>%
  ...
  filter(! c(bmi_at_date_of_research >= 28 & age_at_date_of_research >= 18)) # 426 subjects (rows)
...
```{r}
 sample <- bmi.exclusion %>% 
  ...
  match.data() # 328 entries / subjects after matching (164 each group)
