...
write.csv(dem.table,"tables/demographics_C.csv", row.names = FALSE)
```
...
dem <- demographics_stats_HC_T1 %>%
  ...
  save_kable(file="tables/thalamus_demographics_C.png", zoom=5)
```
...
descrthalamus_join %>%
  ...
  save_kable(file="tables/thalamus_volumes_C.png", zoom=4)
```
