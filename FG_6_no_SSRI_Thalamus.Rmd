...
glmdata_SSRI <- sample %>%
  ...
  filter(type_of_current_medication___1 == "no") %>% # exclude all subjects with antidepressant medication 
...
dem <- demographics_stats_HC_T1 %>%
  ...
  save_kable(file="tables/thalamus_demographics_NoSSRI.png", zoom=5)
```
...
ggsave(barplotarrange.etiv_fdr, filename="figures/barplots_glm_etiv_fdr_NoSSRI.png", width=30, height=25, dpi=300)
```
...
ggsave(barplotarrange.whole_fdr, filename="figures/barplots_glm_whole_fdr_NoSSRI.png", width=25, height=25, dpi=300)
```
...
cordata.rlm1 <- ancova.etiv_fdr %>%
  ...
  right_join(glmdata_SSRI, by="hemi_region") %>%
...
cordata.rlm1 %>%
  ...
  write.csv2("tables/correlation_results_RLM1_NoSSRI.csv", row.names = FALSE)
...
```{r}
 ...
 GLM1.2.result.table <- ancova.whole_fdr %>%
  ...
  dplyr::select(-c(hemi_region)) #%>%
  write.csv2("tables/GLM1+2_results_FDR_NoSSRI.csv", row.names = FALSE)
```
...
GLM1.2.hemi <- GLM1.2.result.table%>%
  ...
  write.csv2("tables/GLM1+2_results_hemi_NoSSRI.csv", row.names = FALSE)
...
#   write.csv("tables/GLM1_hemi_sorted_NoSSRI.csv", row.names = FALSE) ### adjust HERE !!
