...
dem <- demographics_stats_HC_T1 %>%
  ...
  save_kable(file="tables/thalamus_demographics_NoComorb.png", zoom=5)
```
...
ggsave(barplotarrange.etiv_fdr, filename="figures/barplots_glm_etiv_fdr_NoCormorb.png", width=30, height=25, dpi=300)
```
...
ggsave(barplotarrange.whole_fdr, filename="figures/barplots_glm_whole_fdr_NoCormorb.png", width=25, height=25, dpi=300)
```
...
cordata.rlm1 <- ancova.etiv_fdr %>%
  ...
  right_join(glmdata_comorb, by="hemi_region") %>%
...
cordata.rlm1 %>%
  ...
  write.csv2("tables/correlation_results_RLM1_NoComorb.csv", row.names = FALSE)
...
```{r}
 ...
 GLM1.2.result.table <- ancova.whole_fdr %>%
  ...
  dplyr::select(-c(hemi_region)) #%>%
  write.csv2("tables/GLM1+2_results_FDR_NoCormorb.csv", row.names = FALSE)
```
...
GLM1.2.hemi <- GLM1.2.result.table%>%
  ...
  write.csv2("tables/GLM1+2_results_hemi_NoCormorb.csv", row.names = FALSE)
