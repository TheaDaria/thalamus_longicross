...
# library(stringr) # zum string schneiden
# library(htmlTable)
# library(rmarkdown)
# library(Hmisc)
# 
...
### GLM 1c - adjusted for eTIV and age
...
### FDR corection for GLM 1c (etiv) and GLM 2c (etiv + whole thal)
...
ggsave(barplotarrange.etiv_fdr, filename="figures/barplots_glm_etiv_fdr_age+leptin.png", width=30, height=25, dpi=300)
```
...
ggsave(barplotarrange.whole_fdr, filename="figures/barplots_glm_whole_fdr_age+leptin.png", width=25, height=25, dpi=300)
```
...
ggsave(barplotarrange.subcort, filename="figures/barplots_glm_subcort.png", width=30, height=25, dpi=300)
```
...
write.csv2(cordata.rlm1,"tables/RLM_signinuclei_age+leptin.csv", row.names = FALSE)


# cordata.rlm1 %>%
#   dplyr::select(-c(hemi_region, variables_group, nuclei_group)) #%>%
