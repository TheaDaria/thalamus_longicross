...
ancova.subcort <- glmdata %>%
  mutate(point_of_research = relevel(as.factor(point_of_research), ref="HCW")) %>%
  nest(data = - c(region, hemi_region, labeller, group, hemi_group, group_labeller, region_type)) %>%
  mutate(
    lm = map(data, ~ lm(measure ~ point_of_research + e_tiv + subcort_gray, data = .x, na.action = na.omit)),
    adj.means = map(lm, ~ emmeans(.x, "point_of_research")),
    tidymeans = map(adj.means, tidy),
    anova = map(lm, ~ car::Anova(.x, type="III", test.statistic="F")),
    anova_stats = map(anova, ~ sjstats::anova_stats(.x))) %>%
...
#barplotarrange.subcort

#ggsave(barplotarrange.subcort, filename="figures/GLM3a_barplots.png", width=30, height=25, dpi=300)
```
...
ggsave(barplotarrange.etiv_fdr, filename="figures/GLM1a_barplots_fdr.png", width=30, height=25, dpi=300)
```
...
ggsave(barplotarrange.whole_fdr, filename="figures/GLM2a_barplots_fdr.png", width=25, height=25, dpi=300)
```

### FDR corrected GLM 3 Bars
...
ggsave(barplotarrange.subcort_fdr, filename="figures/GLM3a_barplots_fdr.png", width=30, height=25, dpi=300)
```

### Robust regression models ############# using only significant nuclei from GLM 1 & 2 ########################################################


## RLM1a: etiv and age as covariables for predictor, 
## Predictor: ZWANG
...
write.csv(cordata.rlm1,"tables/RLM1a_ZWANG_signinuclei_fromGLM1a.csv", row.names = FALSE)
...
## RLM2a: etiv and age as covariables for predictor, 
## Predictor: LEPTIN
```{r robustregressionnuclei, results="asis", message=FALSE, warning=FALSE}
sample_regression <- sample %>%
...
write.csv(cordata.rlm1,"tables/RLM2a_LEPTIN_signinuclei_fromGLM1a.csv", row.names = FALSE)
```
