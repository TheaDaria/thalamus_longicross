...
### GLM 1b - adjusted for eTIV and age
...
ancova.etiv <- glmdata %>%
  ...
  mutate(
    lm = map(data, ~ lm(measure ~ point_of_research + e_tiv + age_at_date_of_research, data = .x, na.action = na.omit)),
...
### GLM 2b - adjusted for whole thalamus
...
filter(region %in% "Whole_thalamus") %>%
  ...
  mutate(
    lm = map(data, ~ lm(measure ~ point_of_research + e_tiv + whole_thalamus + age_at_date_of_research, data = .x, na.action = na.omit)),
    adj.means = map(lm, ~ emmeans(.x, "point_of_research")),
    tidymeans = map(adj.means, tidy),
    anova = map(lm, ~ car::Anova(.x, type="III", test.statistic="F")),
    anova_stats = map(anova, ~ sjstats::anova_stats(.x))) %>%
...
### FDR corection for GLM 1b (etiv) and GLM 2b (etiv + whole thal)
...
ggsave(barplotarrange.etiv_fdr, filename="figures/barplots_glm_etiv_fdr_age.png", width=30, height=25, dpi=300)
```

### FDR corrected GLM 2b Bars
```{r wholeplot, fig.cap="Barplots for GLMs, adjusted for whole thalamus", echo=TRUE, message=FALSE, warning=FALSE}
for (Region in unique(bardata.whole$region)) {
...
ggsave(barplotarrange.whole_fdr, filename="figures/barplots_glm_whole_fdr_age.png", width=25, height=25, dpi=300)
```
