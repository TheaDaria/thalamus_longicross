...
ancova.subcort <- glmdata %>%
  ...
  nest(data = - c(region, hemi_region, labeller, group, hemi_group, group_labeller, region_type)) %>%
  mutate(
    lm = map(data, ~ lm(measure ~ point_of_research + e_tiv + age_at_date_of_research + subcort_gray, data = .x, na.action = na.omit)),
    adj.means = map(lm, ~ emmeans(.x, "point_of_research")),
    tidymeans = map(adj.means, tidy),
    anova = map(lm, ~ car::Anova(.x, type="III", test.statistic="F")),
    anova_stats = map(anova, ~ sjstats::anova_stats(.x))) %>%
...
ggsave(barplotarrange.etiv_fdr, filename="figures/GLM1b_barplots_fdr.png", width=30, height=25, dpi=300)
```
...
ggsave(barplotarrange.whole_fdr, filename="figures/GLM2b_barplots_fdr.png", width=25, height=25, dpi=300)
```

### FDR corrected GLM 3b Bars
...
ggsave(barplotarrange.subcort_fdr, filename="figures/GLM3b_barplots_fdr.png", width=30, height=25, dpi=300)
```
...
write.csv(cordata.rlm1,"tables/RLM1b_ZWANG_signinuclei_fromGLM1b.csv", row.names = FALSE)
...
write.csv(cordata.rlm1,"tables/RLM2b_LEPTIN_signinuclei_fromGLM1b.csv", row.names = FALSE)
```