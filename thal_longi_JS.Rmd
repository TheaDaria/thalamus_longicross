...
```{r}
setwd("Z:/For_TrDeN/Projekte/Anorexie/05_Projekte/smri/thalamus_subfields/analysis/R_project_smri-Thalamus")
redcap_totaltemp <- read.csv("data/redcap_do_not_delete/redcap.csv")


setwd("//med/zfs/users/SCHLOEMJO/Desktop/Thalamus_R_temp/data")

redcap_total <- read.csv("redcap_leptin_new.csv")%>%
  dplyr::select("participant_id", "redcap_event_name", "research_blood_results_leptin")%>%
  right_join(., redcap_totaltemp, by=c("participant_id", "redcap_event_name"), .keep_all=TRUE)%>%
mutate(point_of_research=factor(point_of_research,
                                  levels=c("T1", "T2", "recAN", "HCW"))) %>%
      rename(research_blood_results_leptin=research_blood_results_leptin.y)


redcap_total$id <- substring(redcap_total$storage_name, 1, 9)
...
Background: >1 T1-scans per subject available from some subjects that have passed our general and thal QC (they participated in >1 research studies) => unique T1-scans must be selected.  
...
uniquesubjects_recAN <- uniquesubjects_prep %>%
  ...
  mutate(order_date=ifelse(is.na(delta_dates), 2,
                             ifelse((abs(delta_dates)<10) & 
                               (delta_dates<0), 1, 2))) %>%
  mutate(demo_date=ifelse(is.na(as.numeric(delta_dates)), as.numeric(date_of_research), 
                            ifelse((abs(as.numeric(delta_dates))<10) & (as.numeric(delta_dates>0)), 
...
#replace missing leptin data & get QRILC 
```{r sample, message=FALSE, warning=FALSE}
sample_is.na_leptin <- postexclusion %>% distinct(storage_name, .keep_all=TRUE) %>%
...
leptin_replace <- inner_join(sample_is.na_leptin, redcap_leptin, by=c("participant_id", "point_of_research")) %>%
  ...
  dplyr::select(storage_name.x, research_blood_results_leptin.y, clinical_blood_results_leptin, delta_dates, leptin2021) %>%
  dplyr::rename(storage_name=storage_name.x) %>%
  distinct(storage_name, .keep_all=TRUE)

leptin_adder <- redcap_total%>%
  dplyr::select(storage_name, research_blood_results_leptin.x)%>%
  rename(research_blood_results_leptin=research_blood_results_leptin.x)

leptinpostexclusion <- postexclusion %>%
    left_join(leptin_adder, by=c("storage_name"))%>%
  left_join(leptin_replace, by="storage_name") %>%
rename(., research_blood_results_leptin=research_blood_results_leptin.y.y)%>%
  mutate(research_blood_results_leptin=coalesce(research_blood_results_leptin, research_blood_results_leptin.y.x))%>%
  ...
  dplyr::select(-c("delta_dates", "research_blood_results_leptin.x", "research_blood_results_leptin.y.x"))
...
setwd("//med/zfs/users/SCHLOEMJO/Desktop/Thalamus_R_temp/data")
QRILC_leptin <- read.csv("QRILC_leptin.csv")%>%
  dplyr::select(research_blood_results_leptin, redcap_event_name, participant_id, leptin_qrilc_batch_m2014, leptin_qrilc_batch_m2017, leptin_qrilc_batch_m2021)

redcap_total.identifier <- redcap_total%>%
  dplyr::select(research_blood_results_leptin, redcap_event_name, participant_id, storage_name, research_blood_results_leptin_batch)

redcap_total.identifier$research_blood_results_leptin_batch <- as.character(redcap_total.identifier$research_blood_results_leptin_batch)

leptinpostexclusion <- left_join(redcap_total.identifier, QRILC_leptin, by=c("research_blood_results_leptin", "redcap_event_name", "participant_id"))%>%
  right_join(leptinpostexclusion, by=c("storage_name", "participant_id", "research_blood_results_leptin"))%>%
  mutate(leptin_qrilc=case_when(research_blood_results_leptin<0.05 & stringr::str_detect(research_blood_results_leptin_batch, "2014") ~ as.character(as.numeric(leptin_qrilc_batch_m2014)),
          research_blood_results_leptin<0.05 & stringr::str_detect(research_blood_results_leptin_batch, "2017") ~ as.character(as.numeric(leptin_qrilc_batch_m2017)),
          research_blood_results_leptin<0.05 & stringr::str_detect(research_blood_results_leptin_batch, "2021") ~ as.character(as.numeric(leptin_qrilc_batch_m2021)),
                                                   TRUE ~ as.character(as.numeric(research_blood_results_leptin))))%>%
  mutate(log_leptin.qrilc=log10(as.numeric(leptin_qrilc)))
  
  leptinpostexclusion <- do.call(data.frame,                      # Replace Inf in data by NA
                   lapply(leptinpostexclusion,
...
match_T1_T2 <- leptinpostexclusion %>%
...
sample <- anti_join(T1_T2_recAN_HC, exclude_reinclusions, by="storage_name") %>%
  ...
  mutate(por=fct_relevel(por, c("acAN", "recAN", "HCW")))%>%
...
sample %>% 
  ...
  group_by(point_of_research.x)%>%
  distinct(id, .keep_all=TRUE)%>%
  mutate(siabex_past_result_an_type=case_when(str_detect(storage_name.x, "64-10-083-2_dehab") ~ 1,               #manually enter 2 missing subtypes
                                         str_detect(storage_name.x, "64-10-337-2_suber") ~ 1,
                                         TRUE ~ as.numeric(as.character(siabex_past_result_an_type))
                                ))%>%
...
descriptives_all <- sample %>%
  distinct(storage_name, .keep_all=TRUE) %>% 
  left_join(redcap_total, by=c("storage_name", "id")) %>% 
...
anovas.filter <- anovas.prep %>% # IQ, Min BMI and trait anxiety are not assessed at acAN-TP2 (TP2 contains "NA" and has to be removed from data.frame for these variables)
  dplyr::filter((variable %in% c("iq", "minbmi", "e_tiv") & (tp %in% "T2")))
...
anovas.lme <- anovas.prep %>%
  filter(!variable %in% c("iq", "minbmi", "e_tiv")) %>%
...
lmedata <- sample %>%
  ...
  dplyr::select(storage_name, participant_id, por, point_of_research, hemi_region, labeller, region, 
                ...
                resultquest_bdi2_total, edi_core, resultquest_scl90r_skagloba, resultquest_scl90r_skazwang, log_leptin.qrilc) %>%
  ...
  mutate(delta.sclzwang.T1_minus_T2 = ifelse(point_of_research %in% "T1", 
                             resultquest_scl90r_skazwang - lead(resultquest_scl90r_skazwang), 
                              0)) %>%
  mutate(delta.log_leptin.T1_minus_T2 = ifelse(point_of_research %in% "T1", 
                              log_leptin.qrilc - lead(log_leptin.qrilc),
                              0))%>%
  ungroup() %>%
...
lme <- lmedata %>%
  ...
  mutate(
    ...
    # Modification 2: Basic LME model with change in leptin concentration (focus on significant nuclei from main!!)
    anova_leptin = map(data, ~ anova(lmerTest::lmer(measure ~ 1 + por + delta.bmisds.T1_minus_T2 +
                                                   age.meandiff + etiv.meandiff + delta.log_leptin.T1_minus_T2 +
                                                     (1|participant_id), 
                                             data=.x, REML = TRUE, na.action = na.omit),
                              type="III", ddf="Satterthwaite")),
...
```
## LME modifications for significant longi nuclei
###Leptin
```{r lmeleptinmichange, results="asis", message=FALSE, warning=FALSE}
lme.leptin <- contrasts.mainlme %>%
  filter((contrast=="acAN-TP2 vs. acAN-TP1")&(p.adj<0.05)) %>%
  dplyr::select(hemi_region) %>%
  left_join(lmedata, by="hemi_region") %>%
  filter_at(vars(por, measure, delta.bmisds.T1_minus_T2, age_at_date_of_research, 
                 e_tiv, delta.leptin.T1_minus_T2), ~!is.na(.x)) %>%
  dplyr::select(-c(storage_name, point_of_research)) %>%
  nest(-c(hemi_region, region, labeller, whole.sub)) %>%
  mutate(
    ...
    lmetest=map(data, ~lmerTest::lmer(measure ~1 + por +
                                        delta.bmisds.T1_minus_T2 +
                                        delta.leptin.T1_minus_T2 +
                                        age.c +
                                        etiv.c + 
                                        (1|participant_id), 
                                      data=.x, REML=TRUE, na.action=na.omit)),
    ...
    #cohensD=map(cohensD, ~filter(.x, term=="delta.leptin.T1_minus_T2")),
    #cohensD=map(cohensD, ~dplyr::select(.x, d)))

lme.leptin %>%
  dplyr::select(labeller, hemi_region, whole.sub, lmetest.tidy) %>%
  unnest(lmetest.tidy) %>%
  filter(grepl("leptin", term)) %>%
  ...
  mutate(d=map(effdata, ~effectsize::t_to_d(t=.x$statistic, df_error=.x$df, paired=FALSE, ci=0.95,
                                      alternative="less"))) %>% # one-sided significance testing
  unnest(effdata, d) %>%
  mutate(d_formatted=ifelse(abs(d)<0.01, "<0.01", formatC(abs(d), format="f", digits=2))) %>%
  arrange(hemi_region) %>%
  #filter(p.value<0.05) %>%
  dplyr::select(labeller, effect, term, estimate, std.error, statistic, p_formatted, signif,
                p.fdr_formatted, adj.signif, d_formatted) %>%
  knitr::kable(format="html", digits=2, align="l", 
               col.names=c("Thalamus (sub-)region", "Effect", "Term", "beta", "se", 
                           "t", "p", "Signif.", "FDR-q", "Adj. Signif.", "Cohen´s d"), 
               caption="LME model including change in blood leptin concentration (acAN-TP2 vs. acAN-TP1)") %>%
  kable_paper(bootstrap_options="striped", full_width=FALSE, position="left", 
              html_font="Arial", font_size="10")
```
...
lme.scl.zwang <- contrasts.mainlme %>%
  filter((contrast=="acAN-TP2 vs. acAN-TP1")&(p.adj<0.05)) %>%
  dplyr::select(hemi_region) %>%
  left_join(lmedata, by="hemi_region") %>%
  filter_at(vars(por, measure, delta.bmisds.mean.T1_minus_T2, age_at_date_of_research, 
                 e_tiv, delta.sclzwang.T1_minus_T2), ~!is.na(.x)) %>%
  dplyr::select(-c(storage_name, point_of_research)) %>%
  nest(-c(hemi_region, region, labeller, whole.sub)) %>%
  mutate(
    data=map(data, ~mutate(.x,
                           ...
                           etiv.c=e_tiv - mean(e_tiv, na.rm=TRUE))),
    lmetest=map(data, ~lmerTest::lmer(measure ~1 + por +
                                        delta.bmisds.mean.T1_minus_T2 +
                                        delta.sclzwang.T1_minus_T2 +
                                        age.c +
                                        etiv.c + 
                                        (1|participant_id), 
                                      data=.x, REML=TRUE, na.action=na.omit)),
    ...
    #                                    data=.x, REML=TRUE, na.action=na.omit), data=.x, type="lme4")),
    #cohensD=map(cohensD, ~rownames_to_column(.x, "term")),
    #cohensD=map(cohensD, ~filter(.x, term=="delta.sclzwang.T1_minus_T2")),
    #cohensD=map(cohensD, ~dplyr::select(.x, d)))

lme.scl.zwang %>%
  dplyr::select(labeller, hemi_region, whole.sub, lmetest.tidy) %>%
  unnest(lmetest.tidy) %>%
  filter(str_detect(term, "sclzwang")) %>%
  mutate(p_formatted=ifelse(p.value<0.001, "<0.001", formatC(p.value, format="f", digits=3))) %>%
  mutate(signif=as.character(asterisk_function(p.value))) %>%
  group_by(whole.sub) %>%
  mutate(p.fdr=p.adjust(p.value, method="fdr")) %>%
  ...
  knitr::kable(format="html", digits=2, align="l", 
               col.names=c("Thalamus (sub-)region", "Effect", "Term", "beta", "se", 
                           "t", "p", "Signif.", "FDR-q", "Adj. Signif.", "Cohen's d"), 
               caption="LME model including change in compulsiveness psychiatric symptoms (acAN-TP2 vs. acAN-TP1)") %>%
  kable_paper(bootstrap_options="striped", full_width=FALSE, position="left", 
              html_font="Arial", font_size="10")
```
### Edi Core
```{r lmeedi, results="asis", message=FALSE, warning=FALSE}
lme.edi.core <- contrasts.mainlme %>%
  filter((contrast=="acAN-TP2 vs. acAN-TP1")&(p.adj<0.05)) %>%
  dplyr::select(hemi_region) %>%
  left_join(lmedata, by="hemi_region") %>%
  filter_at(vars(por, measure, delta.bmisds.mean.T1_minus_T2, age_at_date_of_research, 
                 e_tiv, delta.edicore.T1_minus_T2), ~!is.na(.x)) %>%
  dplyr::select(-c(storage_name, point_of_research)) %>%
  nest(-c(hemi_region, region, labeller, whole.sub)) %>%
  mutate(
    data=map(data, ~mutate(.x,
    ...
    #                                    data=.x, REML=TRUE, na.action=na.omit), data=.x, type="lme4")),
    #cohensD=map(cohensD, ~rownames_to_column(.x, "term")),
    #cohensD=map(cohensD, ~filter(.x, term=="delta.edicore.T1_minus_T2")),
    #cohensD=map(cohensD, ~dplyr::select(.x, d)))

lme.edi.core %>%
  dplyr::select(labeller, hemi_region, whole.sub, lmetest.tidy) %>%
  unnest(lmetest.tidy) %>%
  filter(str_detect(term, "edi")) %>%
  mutate(p_formatted=ifelse(p.value<0.001, "<0.001", formatC(p.value, format="f", digits=3))) %>%
  mutate(signif=as.character(asterisk_function(p.value))) %>%
  group_by(whole.sub) %>%
  mutate(p.fdr=p.adjust(p.value, method="fdr")) %>%
  ...
  knitr::kable(format="html", digits=2, align="l", 
               col.names=c("Thalamus (sub-)region", "Effect", "Term", "beta", "se", 
                           "t", "p", "Signif.", "FDR-q", "Adj. Signif.", "Cohen's d"), 
               caption="LME model including change in EDI core psychiatric symptoms (acAN-TP2 vs. acAN-TP1)") %>%
  kable_paper(bootstrap_options="striped", full_width=FALSE, position="left", 
              html_font="Arial", font_size="10")
```
...
lme.bdi <- contrasts.mainlme %>%
  filter((contrast=="acAN-TP2 vs. acAN-TP1")&(p.adj<0.05)) %>%
  dplyr::select(hemi_region) %>%
  left_join(lmedata, by="hemi_region") %>%
  filter_at(vars(por, measure, delta.bmisds.mean.T1_minus_T2, age_at_date_of_research, 
                 e_tiv, delta.bdi.T1_minus_T2), ~!is.na(.x)) %>%
  dplyr::select(-c(storage_name, point_of_research)) %>%
  nest(-c(hemi_region, region, labeller, whole.sub)) %>%
  mutate(
    data=map(data, ~mutate(.x,
    ...
    #                                    data=.x, REML=TRUE, na.action=na.omit), data=.x, type="lme4")),
    #cohensD=map(cohensD, ~rownames_to_column(.x, "term")),
    #cohensD=map(cohensD, ~filter(.x, term=="delta.bdi.T1_minus_T2")),
    #cohensD=map(cohensD, ~dplyr::select(.x, d)))
...
lme.bdi %>%
  ...
  knitr::kable(format="html", digits=2, align="l", 
               col.names=c("Thalamus (sub-)region", "Effect", "Term", "beta", "se", 
                           "t", "p", "Signif.", "FDR-q", "Adj. Signif.", "Cohen's d"), 
               caption="LME model including change in BDI psychiatric symptoms (acAN-TP2 vs. acAN-TP1)") %>%
  kable_paper(bootstrap_options="striped", full_width=FALSE, position="left", 
              html_font="Arial", font_size="10")
```

###Zwang & BDI
```{r lmebdizwang, results="asis", message=FALSE, warning=FALSE}
lme.zwang.bdi <- contrasts.mainlme %>%
  filter((contrast=="acAN-TP2 vs. acAN-TP1")&(p.adj<0.05)) %>%
  dplyr::select(hemi_region) %>%
  left_join(lmedata, by="hemi_region") %>%
  filter_at(vars(por, measure, delta.bmisds.mean.T1_minus_T2, age_at_date_of_research, 
                 e_tiv, delta.bdi.T1_minus_T2, delta.sclzwang.T1_minus_T2), ~!is.na(.x)) %>%
  dplyr::select(-c(storage_name, point_of_research)) %>%
  nest(-c(hemi_region, region, labeller, whole.sub)) %>%
  mutate(
    data=map(data, ~mutate(.x,
                           ...
                           etiv.c=e_tiv - mean(e_tiv, na.rm=TRUE))),
    lmetest=map(data, ~lmerTest::lmer(measure ~1 + por +
                                        delta.bmisds.mean.T1_minus_T2 + delta.sclzwang.T1_minus_T2 +
                                        delta.bdi.T1_minus_T2 +
                                        age.c +
                                        etiv.c + 
                                        (1|participant_id), 
...
lme.zwang.bdi %>%
  ...
  knitr::kable(format="html", digits=2, align="l", 
               col.names=c("Thalamus (sub-)region", "Effect", "Term", "beta", "se", 
                           "t", "p", "Signif.", "FDR-q", "Adj. Signif.", "Cohen's d"), 
               caption="LME model including change in SCL90-R Compulsiveness and BDI psychiatric symptoms (acAN-TP2 vs. acAN-TP1)") %>%
  kable_paper(bootstrap_options="striped", full_width=FALSE, position="left", 
              html_font="Arial", font_size="10")
...
R.lmedata <- lmedata %>%
  left_join(redcap_totalstore_AN, by=c("storage_name", "point_of_research")) %>%
  mutate(subtype=ifelse(point_of_research=="T1", siabex_result_an_type, 0)) %>%
  mutate(past_subtype=ifelse(point_of_research=="recAN", siabex_past_result_an_type, 0)) %>%
  group_by(hemi_region, participant_id) %>% arrange(point_of_research, .by_group=TRUE) %>%
  mutate(subtype=ifelse(point_of_research=="T2",
                       lag(subtype), subtype)) %>%
  ungroup() %>%
  filter(subtype!=2) %>% # exclude binge/purge in acAN
  filter(past_subtype!=2)%>% #exclude binge/purge in recAN
...
R.contrasts <- R.lme %>%
  dplyr::select(hemi_region, region, labeller, whole.sub, contrasts.tidy) %>%
...
R.contrasts %>%
  filter(p.value<0.05) %>%
  dplyr::select(labeller, contrast, term, p_formatted, signif, 
                p.fdr_formatted, adj.signif) %>%
  knitr::kable(format="html", align="l", col.names=c("Thalamus (sub-)region", "Contrast", "Term", 
                                                     "p", "Signif.",
                                                     "FDR-q", "Adj. signif."),
...
for (Hemi_Region in unique(R.lme$hemi_region)) {
  ...
  cohensD.R <- R.lme %>%
    dplyr::select(hemi_region, region, labeller, emmeans) %>%
    filter(hemi_region %in% Hemi_Region) %>%
    mutate(
    ...
    dplyr::select(hemi_region, region, labeller, contrast, cohensD.R_pre,
                  cohensD.R, d_formatted, se, confint_95)

  cohenlist.R[[Hemi_Region]]=cohensD.R
  }

cohensD.R.lme <- as.tibble(bind_rows(cohenlist.R[1:52]))
                         
cohensD.R.lme %>%
  arrange(hemi_region) %>%
  dplyr::select(labeller, contrast, cohensD.R_pre, se, confint_95) %>%
  knitr::kable(format="html", align="l", col.names=c("Thalamus (sub-)region", "Contrast", "Cohen's d", "se", "95% CI"), 
...
R.bardata <- R.lme %>%
  dplyr::select(hemi_region, region, labeller, whole.sub, emmeans.tidy) %>%
  unnest(emmeans.tidy) %>%
  ...
  mutate(emmeans=factor(emmeans, levels=c("AcAN-TP1", "AcAN-TP2", "zerobar1", 
                                          ...
                                          "HC"))) %>%
  arrange(region) %>%
  mutate(sigsize=ifelse(region=="Whole_thalamus", 18, 14)) %>%
  mutate(effsize=ifelse(region=="Whole_thalamus", 12, 10)) %>%
  mutate(stripsize=ifelse(region=="Whole_thalamus", 35, 35)) %>%
  mutate(ysize=ifelse(region=="Whole_thalamus", 45, 35)) %>%
  mutate(xlabels=ifelse(region=="Whole_thalamus", TRUE, FALSE))%>%
mutate(region=case_when(str_detect(region, "Mv(Re)") ~ "Mv_Re",
                                         str_detect(region, "L-Sg") ~ "L_Sg",
       TRUE ~ as.character(region)))   #rename invalid variable names
  ...
  y.position <- R.bardata %>% 
   ...
   longi.line <- R.bardata %>% 
    ...
    mutate(y=dplyr::recode(emmeans,
                                   "AcAN-TP1"="y",
                                   "AcAN-TP2"="yend")) %>% 
    dplyr::select(labeller, y, estimate) %>%
...
bp.whole.R1 <- plotlist$Whole_thalamus
bp.nuclei.R2 <- plotlist$AV
bp.nuclei.R3 <- plotlist$LD
bp.nuclei.R4 <- plotlist$LP
bp.nuclei.R5 <- plotlist$VA
bp.nuclei.R6 <- plotlist$VAmc
bp.nuclei.R7 <- plotlist$VLa
bp.nuclei.R8 <- plotlist$VLp
bp.nuclei.R9 <- plotlist$VPL
bp.nuclei.R10 <- plotlist$VM
bp.nuclei.R11 <- plotlist$CeM
bp.nuclei.R12 <- plotlist$CL
bp.nuclei.R13 <- plotlist$Pc
bp.nuclei.R14 <- plotlist$CM
bp.nuclei.R15 <- plotlist$Pf
bp.nuclei.R16 <- plotlist$Pt
bp.nuclei.R17 <- plotlist$MV_Re
bp.nuclei.R18 <- plotlist$MDm
bp.nuclei.R19 <- plotlist$MDl
bp.nuclei.R20 <- plotlist$LGN
bp.nuclei.R21 <- plotlist$MGN
bp.nuclei.R22 <- plotlist$L_Sg
bp.nuclei.R23 <- plotlist$PuA
bp.nuclei.R24 <- plotlist$PuM
bp.nuclei.R25 <- plotlist$PuL
bp.nuclei.R26 <- plotlist$PuI
...
barplotarrange.row1.R <- ggarrange(bp.nuclei.R2,
                                 bp.nuclei.R3,
                                 bp.nuclei.R4, bp.nuclei.R5,
                                 bp.nuclei.R6, ncol=5, nrow=1)
#ggsave(barplotarrange.row1.R, path="figures", filename="Rlme.row1.png", 
       #width=32, height=10, dpi=600)

barplotarrange.row2.R <- ggarrange(bp.nuclei.R7,
                                 bp.nuclei.R8,
                                 bp.nuclei.R9, bp.nuclei.R10,
                                 bp.nuclei.R11, ncol=5, nrow=1)
#ggsave(barplotarrange.row2.R, path="figures", filename="Rlme.row2.png", 
       #width=32, height=10, dpi=600)

barplotarrange.row3.R <- ggarrange(bp.nuclei.R12,
                                 bp.nuclei.R13,
                                 bp.nuclei.R14, bp.nuclei.R15,
                                 bp.nuclei.R16, ncol=5, nrow=1)
#ggsave(barplotarrange.row3.R, path="figures", filename="Rlme.row3.png", 
       #width=32, height=10, dpi=600)

barplotarrange.row4.R <- ggarrange(bp.nuclei.R17,
                                 bp.nuclei.R18,
                                 bp.nuclei.R19, bp.nuclei.R20,
                                 bp.nuclei.R21, ncol=5, nrow=1)
#ggsave(barplotarrange.row4.R, path="figures", filename="Rlme.row3.png", 
       #width=32, height=10, dpi=600)

barplotarrange.row5.R <- ggarrange(bp.nuclei.R22,
                                 bp.nuclei.R23,
                                 bp.nuclei.R24, bp.nuclei.R25,
                                 bp.nuclei.R26, ncol=5, nrow=1)
#ggsave(barplotarrange.row4.R, path="figures", filename="Rlme.row3.png", 
       #width=32, height=10, dpi=600)
```
...
for (Hemi_Region in unique(excl.lme$hemi_region)) {
  ...
  cohensD.excl <- excl.lme %>%
    ...
    unite("confint_95", c(CI_95_lower, CI_95_upper), sep=",", remove=FALSE, na.rm=FALSE) %>%
    unite("confint_95", c(bracket01, confint_95, bracket02), sep="", remove=FALSE, na.rm=FALSE) %>%
    dplyr::select(hemi_region, region, labeller, contrast, cohensD.excl_pre,
                  cohensD.excl, d_formatted, se, confint_95)

  cohenlist.excl[[Hemi_Region]]=cohensD.excl
  }

cohensD.excl.lme <- as.tibble(bind_rows(cohenlist.excl[1:52]))
                         
cohensD.excl.lme %>%
  arrange(hemi_region) %>%
  dplyr::select(labeller, contrast, cohensD.excl_pre, se, confint_95) %>%
  knitr::kable(format="html", align="l", col.names=c("Thalamus (sub-)region", "Contrast", "Cohen's d", "se", "95% CI"), 
...
cohen.d.mainlme2 %>%
  ...
  arrange(contrast)%>%
  knitr::kable(format = "html", align = "l", col.names = c("Thalamus (sub)region", "Contrast (pairwise group differences)", "p-value (FDR-adjusted)", "Adjusted significance", "Effect size: Cohen's d"), 
               caption = "LME model 2 for thalamus nuclei volumes: Contrasts table including effect size statistics") %>%
  kable_paper(bootstrap_options="striped", full_width=FALSE, position="left", html_font="Arial", font_size="10") %>%
  column_spec(5, bold=TRUE) %>%
...
contrasts.mainlme.wholethal %>%
  ...
  mutate(
    ...
    rlmtest = map(rlm, ~ f.robftest(.x, var="measure")), # predictor = thalamus volume
  ...
  #add_header_above(c("No significant relationships between thalamus nuclei volumes significantly different from HC according to main LME model and any of the assessed clinical measures (BMI-SDS, leptin concentration, psychiatric symptoms) at acAN-TP2."=19), align="l") #%>%
